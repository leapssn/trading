...
const Strategies = (() => {
  ...
  const BUILT_IN = [
    {
      id: 'sr', name: 'Support / Résistance', icon: Icons.ruler,
...
- Les zones testées plusieurs fois sont plus fortes`,
    ...
    {
      id: 'smc', name: 'Smart Money Concepts (SMC)', icon: Icons.bank,
...
5. Entrer en retest de l'OB, stop sous le bas de l'OB`,
    ...
    {
      id: 'breakout', name: 'Breakout / Cassure', icon: Icons.rocket,
...
- Éviter les breakouts en fin de session`,
    ...
    {
      id: 'trend', name: 'Trading dans la Tendance', icon: Icons.trendUp,
...
- H1/M15 : timing`,
    ...
    {
      id: 'rr', name: 'Gestion du Risque & R:R', icon: Icons.scale,
...
| 1:5 | 17% |`,
  ...
  function stratCard(s, isCustom) {
    const actions = isCustom ? `
      <div class="flex gap-3 mt-3">
        <button onclick="event.stopPropagation();Strategies.openEdit('${s.id}')" class="text-xs hover:text-white flex items-center gap-1" style="color:var(--text-faint)">${Icons.pencil} Modifier</button>
        <button onclick="event.stopPropagation();Strategies.deleteCustom('${s.id}')" class="text-xs hover:text-red-400 flex items-center gap-1" style="color:var(--text-faint)">${Icons.trash} Supprimer</button>
      </div>` : '';
    return `
      <div class="strat-card" onclick="Strategies.show('${s.id}', ${isCustom})">
        <div class="flex items-center gap-3 mb-2">
          <span style="color:var(--brand)">${s.icon || Icons.file}</span>
  ...
  async function saveCustom() {
    ...
    const s = { id: id || Store.uid(), name, content, icon: Icons.file, createdAt: new Date().toISOString() };
